    ...
    window.Framework7 = function (params) {
		...
		app.ajaxGetData = function(config){         
            ...
            ajax(url,data,function(data_,tips){
                        ...
                        if( !data_['response']['trades'] ){
                            data_['response']['trades'] = data_['response']['order']
                        }
                        SMK.C('当前是第:' + current.val() + '页     请求条数：'+data.pageSize)
                        if(data_['response']['trades'].length == 0 && currentPage==1){
                            data_['response']['Result'] = false;
                        }
                        else{
                            data_['response']['Result'] = true
                        }
                        max.val(data_['totalpage']);    
                        var odate = data_['response']['trades'];
                        for(i in odate){
                           if(!odate[i]['odate']){
                                  odate[i]['odate'] = odate[i]['ridingtime'];
                           }
                           if(odate[i]['odate'].indexOf(getyyyyMMdd())==0){
                                odate[i]['odate_'] = '今天'+odate[i]['odate'].slice(8,10)+':'+odate[i]['odate'].slice(10,12);
                           }
                           else{
                                odate[i]['odate_'] = odate[i]['odate'];
                           }
                        }
                        data_['response']['trades'] = odate;
                        html = SMK.templates[view](data_['response']);
                        if(LASTHTML == html && data_['response']['trades'].length != 0){
                           // showTips('您的点击速度已突破天际')
                           // 在测试阶段由于mock数据一直是死的，所以允许重复
                           // return;
                        }
                        LASTHTML = html;
                        // 添加新条目
                        wrap.append(html);
                        if(data_['response']['Result'] && data_['response']['trades'].length < data.pageSize && data_['response']['trades'].length >= 0){
                            console.log(tips);
                           // showTips('没有更多啦！');
                        ...
                        var empty = el.find('.scroll_content_current .none').parent();
                        if(empty.length>1){
		...
		app.ajaxNextPage = function (config){
			...
			var data = config.data || function(){return{}},
				...
				// 注册'infinite'事件处理函数
				$$(el).find('.infinite-scroll').on('infinite', function (e) {
					// 上次加载的序号
                  current = el.find('.scroll_content_current input.currentPage');  
        ...
        ************   Picker   ************
        ...
        var Picker = function (params) {
            ...
            function closeOnHTMLClick(e) {
                window.picker_cancel = false;
                if (inPopover()) return;
                if (p.input && p.input.length > 0) {  
                    if (e.target !== p.input[0] && $(e.target).parents('.picker-modal').length === 0){
                        window.picker_cancel = true; //在picker容器外部点击，并且不是唤起它的元素，直接设置为true，防止执行onClose回调函数
                        p.close();
                    }
                }
                else {
            ...
            function onPickerClose() {
                ...
                if(!picker_cancel){ //判断是否在picker容器外部点击，并且不是唤起它的元素，只有点击完成按钮才执行onClose回调函数
